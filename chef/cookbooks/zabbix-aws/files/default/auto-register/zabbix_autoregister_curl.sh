...
DNS=`curl --connect-timeout 15 -s http://169.254.169.254/latest/meta-data/hostname`
ISID=`curl --connect-timeout 15 -s http://169.254.169.254/latest/meta-data/instance-id`
...
GRPS=`curl --connect-timeout 15 -s -X POST -H "Content-Type: application/json" "$URL" -d '{"jsonrpc":"2.0","method":"hostgroup.get","params":{"filter":{"name": ["'"$MYTAG"'","Discovered hosts"]}},"id": 1,"auth":"'"$AUTH"'"}' |sed -n 's/.*"result":\[\([^]]*\)\].*/\1/p'`
...
#building api's json
REGISTER=`curl --connect-timeout 15 -s -X POST -H "Content-Type: application/json" "$URL" -d '{
  "jsonrpc":"2.0",
  "method" : "host.create",
  "params":{
      "host":"'"$ISID"'",
